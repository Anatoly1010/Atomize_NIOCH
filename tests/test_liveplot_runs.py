...
@pytest.fixture
def liveplot(monkeypatch, tmp_path, plot_app):
    ...
    window.add_new_plot = MethodType(MainExtended.add_new_plot, window)
...
def test_track_freezes_displayed_manual_phase_correction(liveplot, monkeypatch):
    from atomize.main import widgets_invert

    monkeypatch.setattr(widgets_invert.inv_par, 'detection_freq', lambda: 125.0)
    monkeypatch.setattr(widgets_invert.inv_par, 'set_phase', lambda *args: None)
    source = QObject()
    push_iq(liveplot, source)
    dock = liveplot.namelist['Dig']
    assert isinstance(dock, widgets_invert.InvertCrosshairDock)
    monkeypatch.setattr(dock, 'flash_curve', lambda *args, **kwargs: None)
    dock._rotate_curve_pair(dock.curves['ch'])
    displayed = [dock.curves[label].yData.copy() for label in ('ch', 'ch_1')]
    assert dock.capture_track(('ch', 'ch_1'), ('test', 7))
    dock._rotate_curve_pair(dock.curves['ch'])
    push_iq(liveplot, source, offset=10)
    for reference, saved in zip(dock.track_curves, displayed):
        np.testing.assert_array_equal(reference.yData, saved)
    np.testing.assert_array_equal(dock.curves['ch'].yData, np.arange(5) + 10)
