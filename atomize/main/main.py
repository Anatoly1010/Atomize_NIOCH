...
from atomize.main.main_window import MainWindow, NameList, TAB_MARGINS
from atomize.main import widgets_invert
from atomize.general_modules.gui_style import apply_app_style, CHECKBOX_STYLE, REFINED_STYLES
...
class MainExtended(MainWindow):
        ...
        *lines, tail = pending.split('\n')
        ...
        for line in lines:
            ...
            # Insys stdOut
                ...
                # Drop vendor-library chatter that some device modules emit
                # straight to stdout on import (e.g. pyspcm's "Python Version:
                # ..." banner from the Spectrum AWG/digitizer modules).
    ...
    def _cc_output_finished(self, *args):
        ...
        if leftover.startswith("print "):
            self.text_errors.appendPlainText(leftover[6:].strip())
                ...
                and not leftover.lstrip().startswith(self.suppressed_stdout_prefixes)):
    ...
    def set_control_center(self):
        ...
        groups = [
            ...
            ('Instrument controls', [
                ...
                ('Oscilloscope', self.start_osc_control, 'osc21'),
    ...
    def start_rect_phasing(self, preset=None):
        ...
        """
        args = [os.path.join('..','atomize/control_center/phasing.py')]
    ...
    def start_awg_phasing(self, preset=None):
        ...
        """
        args = [os.path.join('..','atomize/control_center/awg_phasing.py')]
        if preset and isinstance(preset, str):
            args.append(preset)
