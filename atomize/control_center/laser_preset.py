...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        gui_path = os.path.join(path_to_main,'gui/laser_main_window.ui')
    ...
    def design(self):
        ...
        # Spinboxes
        self.box_laser.valueChanged.connect(self.laser)
        self.box_laser.setStyleSheet("QSpinBox { color : rgb(193, 202, 227); }")
        self.cur_laser = int( self.box_laser.value() )
        self.box_laser.lineEdit().setReadOnly( True )
    ...
    def laser_check(self, number):
        # Unavailable positions of MW:
        # 208 - 232 ns
        # 152 - 182 ns
        # 102 - 126 ns
        # 76 is min
        if number < 232 and number > 208:
            if number != 210:
                number = 208
            else:
                number = 232
        elif number < 182 and number > 152:
            if number != 154:
                number = 152
            else:
                number = 182
        elif number < 126 and number > 102:
            if number != 104:
                number = 102
            else:
                number = 126
    ...
    def laser(self):
        self.cur_laser = int( self.box_laser.value() )
        self.cur_laser = self.laser_check( self.cur_laser )
        self.box_laser.setValue( self.cur_laser )
        # LASER TRIGGER LENGTH 50 ns
    ...
    def start(self):
        ...
        self.exp_process = Process( target = self.worker.exp_on, args = ( self.child_conn, self.cur_curve_name, self.cur_exp_name, \
                                            ...
                                            self.cur_end_field, self.cur_step_field, self.cur_averages, self.cur_graph, self.cur_laser, ) )
...
class Worker(QWidget):
    ...
    def exp_on(self, conn, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12):
        ...
        # [              5,                 6,             7,                  8,                   9,                10,             11,             12 ]
        #self.cur_st_field, self.cur_rep_rate, self.cur_scan, self.cur_end_field, self.cur_step_field, self.cur_averages, self.cur_graph, self.cur_delta
