...
class MainWindow(QMainWindow):
    ...
    def design_tab_1(self):
        ...
        pulses = [(QDoubleSpinBox, 0, 100e6, 0, 2, 1, " ns", "_st", "_start"),
                  ...
                  (QDoubleSpinBox, 0, 1900, 0, 2, 1, " ns", "_sig", "_sigma"),
        ...
        self.button_track.setToolTip(
            ...
            "Works in the running preview; references survive stop/restart.")
    ...
    def design_tab_2(self):
        ...
        labels = [("Acquisitions", "label_17"), ("Integration Left", "label_18"), ("Integration Right", "label_19"), ("Detection Points", "label_20"), ("Horizontal Offset", "label_21"), ("Shift Together", "label_shift"), ("Points", "label_e1"), ("Scans", "label_e2"), ("Experiment Name", "label_e3"), ("Curve Name", "label_e4"), ("Start Field", "label_f1"), ("End Field", "label_f2"), ("Field Step", "label_f3"), ("Sweep Type", "label_c1"), ("Start Log Time", "label_e5"), ("End Log Time", "label_e6"),
            ...
            ("Amplitude Step", "label_f4"), ("Cycles", "label_cyc"), ("Save Each Cycle", "label_save_cyc"), ("Auto Window", "label_aw")]
        ...
        double_boxes = [(QSpinBox, "Acq_number", "number_averages", self.acq_number, 1, 1e4, 1, 1, 0, ""),
                      ...
                      (SnapSpinBox, "Hor_offset", "posttrigger", self.hor_offset, 0, 20000, 384, 32, 0, ""),
    ...
    def design_tab_3(self):
        ...
        gridLayout.addWidget(zo_controls, 5, 0, 1, 2)
    ...
    def design_tab_6(self):
        ...
        gridLayout.addWidget(save2d_label, 3, 0)
        gridLayout.addWidget(self.Save2D, 3, 1)
        gridLayout.addWidget(hdf5_label, 4, 0)
        gridLayout.addWidget(self.Save_hdf5, 4, 1)
    ...
    def _structure_sig(self, snap):
        ...
        sigma / type all re-arm live. A LASER on/off is caught via laser_flag, and
    ...
    def _validate_live_edit(self):
        ...
        for i in range(1, 10):
            ...
            if i == receiver and not any(c in raw.lower() for c in 'xy'):
                # coherence-order coefficients, one per pulse, not a phase list
                for t in toks:
                    if coeff.fullmatch(t) is None:
    ...
    def live_apply(self):
        ...
        if self._structure_sig(snap) != self.live_sig:
            ...
            self.errors.appendPlainText(
                ...
                'start or source / phase cycle / pulse count / detection window) — '
    ...
    def update_pulse_type(self, index):
        ...
        # shape atomically). A LASER on/off instead changes laser_flag (the laser
        # row count), which is in the structure signature, so that case
        # auto-restarts.
        self.schedule_live_apply()

    def _apply_laser_row(self, index):
        """Enable/disable the MW-only widgets of a LASER row and set its Length range."""
        ...
        self._linking = True
        try:
            getattr(self, f'P{index}_len').setMaximum(15000 if is_laser else 1900)
        finally:
            self._linking = False
        self._seed_link_prev()
    ...
    def open_file(self, filename):
        ...
        # overwrite the GUI and leak digitizer/field commands (PO/HO/FI/NA) into
        # the running experiment's command pipe. Only valid when idle or
        # previewing (Run Pulses).
        ...
        self.Hor_offset.setValue( int( float( lines[18].split(':  ')[1] ) ) )
...
class Worker():
    ...
    def dig_on(self, conn, dig_points, posttrigger, n_averages, win_left, win_right, rect1, rect2, rect3, rect4, rect5, rect6, rect7, n_wurst, rep_rate, mag_field, fft_flag, cur_phase, ch0_ampl, ch1_ampl, trig_delay, awg2, awg3, awg4, awg5, awg6, awg7, quad, zero_order, first_order, second_order, p_to_drop, b_sech, combo_cor, combo_synt, _reserved, rect8, rect9, awg8, awg9, laser_flag, laser_num, laser_qsw_delay, iq_corr, script_test=False ):
        ...
        try:
            ...
            dig = spectrum.Spectrum_M4I_4450_X8() if DIG_PRESENT else _NullDigitizer()
            ...
            for i, (tp, ap) in enumerate(zip(trigger_pulses, awg_params)):
                if int(float(tp[1].split(' ')[0])) != 0:
                    ...
                    # per-pulse 'AWG' amp-gate marker; the single sequence trigger is P0 (TRIGGER_AWG)
                    if ap[0] != 'BLANK':
                        pb.pulser_pulse(
                            name=f'P{2*i + 3}',
                            channel='AWG',
            ...
            while self.command != 'exit':
                ...
                if iq_cor == 1 or quad == 0:
                    data_x, data_y = dig.digitizer_demodulate(data_x, data_y, iq_freq if iq_cor == 1 else 0,
                ...
                if auto_phase_req:
                    ...
                    if quad == 0 and win_right > win_left:
                        ...
                        if np.isfinite(integral) and integral != 0:
                            ...
                            conn.send(('AutoPhase', (float(np.degrees(phase)), float(np.abs(integral) * sample_ns))))
                        else:
                ...
                if auto_window_ns > 0:
                    width = min(max(1, int(round(auto_window_ns / sample_ns))), WIN_ADC)
                    auto_window_ns = 0
        ...
        try:
            ...
            for i, (tp, ap) in enumerate(zip(trigger_pulses, awg_params)):
                ...
                if int(float(tp[1].split(' ')[0])) != 0:
                    ...
                    awg.awg_pulse(**awg_kwargs)
                    _eseem_add_awg(f'P{2*i + 2}', i + 1 + laser_flag, ap[8])
    ...
    def exp_log(self, conn, decimation, num_ave, scans, points,
        ...
        try:
            ...
            if 'P1' in name_list:
                pb.pulser_redefine_delta_start(name = 'P1', delta_start = f"{self.round_to_closest( nonlinear_diff[0] * rel_shift[0], 2 )} ns")
            ####
            ...
            for n, lr in enumerate(laser_rects, start=1):
                ...
                if f'L{n}' in name_list:
                    laser_delta_start = f"{self.round_to_closest( nonlinear_diff[0] * rel_shift[rs_idx], 2 )} ns"
                    rs_idx += 1
                else:
                    ...
                    for j in range(POINTS):
                        ...
                        if j > 0:
                            ...
                            for nm, x in zip(awg_name_list, awg_rel_shift):
                                awg.awg_redefine_delta_start(name = nm, delta_start = f"{self.round_to_closest(x * new_delta_start, 2)} ns")
