...
class MainWindow(QMainWindow):
    ...
    def design(self):
        ...
        # ---- Boxes ----
        double_boxes = [(QDoubleSpinBox, "box_length", "cur_length", self.pulse_length, 2.0, 1900, 100.0, 2.0, 1, " ns"),
        ...
        text_edit = [("Tune", "text_edit_exp_name", "cur_exp_name", self.exp_name),
                     ("CH1", "text_comment", "cur_comment", self.comment),
                     ("CH2", "text_comment_2", "cur_comment_2", self.comment),
                     ("172.16.16.21", "text_comment_3", "cur_comment_3", self.comment)
                    ]
    ...
    def pulse_length(self):
        ...
        self.cur_length = self.round_and_change(self.box_length, 2.0)
    ...
    def open_dialog(self):
        file_data = self.file_handler.create_file_dialog(multiprocessing = True,
            directory = ldir.load('tune', self.path))
...
class Worker():
    ...
    def exp_on(self, conn, p2, p4, p5, p6, p7, p8, p9, p10):
        ...
        try:
            ...
            import atomize.device_modules.SR_PTC_10 as ptc_mod
            import atomize.general_modules.csv_opener_saver as openfile
            ...
            # setting pulses:
            # DETECTION pulse triggers/gates the oscilloscope, MW pulse drives the bridge.
            pb.pulser_pulse(name ='P0', channel = 'DETECTION', start = PULSE_1_START, length = PULSE_1_LENGTH)
...
def main():
    ...
    apply_app_style(app, app_id='Atomize.NIOCH.TunePreset', desktop='tune')
    main = MainWindow()
    main.show()
    sys.exit(app.exec())
