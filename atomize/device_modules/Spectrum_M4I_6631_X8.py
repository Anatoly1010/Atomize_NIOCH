...
class Spectrum_M4I_6631_X8:
    def __init__(self):
        ...
        self.function_dict = {'SINE': 0, 'GAUSS': 1, 'SINC': 2, 'BLANK': 3, }
        ...
        self.phase_shift_ch1_seq_mode = int(float(self.specific_parameters['ch1_phase_shift'])) # in radians
    ...
    def awg_pulse(self, name = 'P0', channel = 'CH0', func = 'SINE', frequency = '200 MHz', phase = 0,\
     delta_phase = 0, length = '16 ns', sigma = '16 ns', increment = '0 ns', start = '0 ns', delta_start = '0 ns', d_coef = 1):
        ...
        if self.test_flag != 'test':
            ...
            # Function type
            temp_func = str(func)
            assert (temp_func in self.function_dict), 'Incorrect pulse type. Only SINE, GAUSS, SINC, and BLANK pulses are available'
    ...
    def awg_pulse_sequence(self, *, pulse_type, pulse_start, pulse_delta_start,\
        ...
        if self.test_flag != 'test':
                            ...
                            for i in range(pulse_start_smp[index] + pulse_delta_start_smp[index]*point, segment_length):
                                    ...
                                    if i < (pulse_start_smp[index] + pulse_delta_start_smp[index]*point):
                                        ...
                                        if pulse_phase_np[index] != 1000:
                                            ...
                                            pnBuffer[i] = int( self.maxCAD * sin(2*pi*(( i ))*pulse_frequency[index] / self.sample_rate + pulse_phase_np[index] ) )

                                        else:
                                            #self.full_buffer[point, i] 
                                            pnBuffer[i] = int( self.maxCAD * sin(2*pi*(( i ))*pulse_frequency[index] / self.sample_rate + rnd_phase) )
                                    else:
                                        break
                            ...
                            for i in range(2 * (pulse_start_smp[index] + pulse_delta_start_smp[index]*point), 2*segment_length, 2):
                                    if i < 2 * (pulse_start_smp[index] + pulse_delta_start_smp[index]*point):
                                        ...
                                        if pulse_phase_np[index] != 1000:
                                            ...
                                            #self.full_buffer[point, i]
                                            #ch0
                                            pnBuffer[i] = int( self.maxCAD * sin(2*pi*(( i/2 ))*pulse_frequency[index] / self.sample_rate + rnd_phase) )
                                            #ch1
                                            pnBuffer[i + 1] = int( self.maxCAD * sin(2*pi*(( ( i )/2 ))*pulse_frequency[index] / self.sample_rate + rnd_phase + \
                                                                     self.phase_shift_ch1_seq_mode) )                                            
                                    else:
                                        break
            ...
            for el in arguments_array[5]:
                assert( el >= self.min_freq and el <= self.max_freq), 'Pulse frequency should be from 1 to 280 MHz'
    ...
    def convertion_to_numpy(self, p_array):
        ...
        if self.test_flag != 'test':
            ...
            while i < num_pulses:
                ...
                pulse_temp_array.append( (ch_num, func, freq_mhz, phase, leng_time, sig_time, st_time, del_st_time, amp) )
            ...
            for element in pulses:
                ...
                for index2, element2 in enumerate( element ):
                    ...
                    for i in range ( self.segment_memsize*index2, self.segment_memsize*(index2 + 1), 1):
                                             #[      0,        1,               2,           3,                4,               5,     6,           7,               8]
                        if element2[1] == 0: # SINE; [channel, function, frequency (MHz), phase, length (samples), sigma (samples), start, delta_start, amp_coefficient]
    ...
    def define_buffer_single_joined(self):
        ...
        """
        # pulses are in a form [channel_number, function, frequency, phase, length, sigma, start, delta_start, amp] 
        #                      [0,               1,          2,      3,      4,      5,      6,      7,        8   ]
