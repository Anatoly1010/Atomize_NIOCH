...
class MainWindow(QMainWindow):
    ...
    def design_tab_1(self):
        ...
        icon_path = os.path.join(path_to_main, 'gui/icon_rect.ico')
        ...
        self.label_7.setFixedSize(124, 26)
        ...
        try:
            for j in range(1, 10):
                ...
                if suffix in ('_st', '_len'):
                    grid = 2  # RECT pulse time grid (ns)
    ...
    def live_apply(self):
        ...
        if self._structure_sig(snap) != self.live_sig:
            ...
            self.errors.appendPlainText(
                ...
                'phase cycle / pulse count / detection pulse) — full restart performed.')
            return
    ...
    def decimat(self):
        ...
        (horizontal offset) is set independently via the Horizontal Offset control; it
    ...
    def hor_offset(self):
        ...
        points. It must stay smaller than the record length (Detection Points).
...
class Worker():
    ...
    def dig_on(self, conn, dig_points, posttrigger, n_averages, win_left, win_right, rect1, rect2, rect3, rect4, rect5, rect6, rect7, laser_flag, rep_rate, mag_field, fft_flag, quad, zero_order, first_order, second_order, p_to_drop, laser_num, laser_qsw_delay, rect8, rect9, script_test=False):
        ...
        try:
                ...
                if self.command[0:2] == 'PO':
                    ...
                    if snap is not None:
                        def _edited_start(idx, entry):
                            # Mirror the setup: non-laser pulses get the Q-switch delay shift (2 ns grid).
                        ...
                        try:
                            ...
                            for idx, entry in enumerate(snap):
                                ...
                                kwargs = {
                                    'name': f'P{idx}',
                                    'channel': self._laser_channel(idx, laser_flag) or entry[0],
                                    'start': _edited_start(idx, entry),
                                    'length': entry[2],
                                }
                                if self._laser_channel(idx, laser_flag) is None:
            ...
            # path (normal, Stop, or exception): stop+close the digitizer and stop
            # the pulse generator. Each call is guarded (and the device close
            # methods are idempotent), so cleanup for a device that was never
            # opened cannot mask the original error.
                ...
                if script_test:
                    ...
                    if save_each:
                        for idx in range(len(cycle_snapshots)):
                            ...
                            file_handler.save_data(cpath, np.c_[x_axis_plot, cdat[0], cdat[1]], header = header, mode = 'w')
    ...
    def exp_log(self, conn, decimation, num_ave, scans, points,
        ...
        try:
            ...
            # rel_shift has one entry per non-zero pulse; rs_idx tracks its slot in build order
            rs_idx = 0
            for i, p in enumerate(pulses):
                laser_ch = self._laser_channel(i, laser_flag)
                if laser_flag >= 1 and laser_ch is None:
...
def main():
    ...
    apply_app_style(app, app_id='Atomize.ITC.Phasing', desktop='rect')
    main = MainWindow()
    main.show()
